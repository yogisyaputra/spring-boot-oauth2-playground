...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // Normalisasi data dari provider
        ...
        if ("google".equals(registrationId)) {
            ...
            email = (String) attributes.get("email"); // bisa null kalau user privat email
            name = (String) attributes.getOrDefault("name", attributes.get("login"));
            avatarUrl = (String) attributes.get("avatar_url");
            username = (String) attributes.get("login");
            profileUrl = (String) attributes.get("html_url");

            // kalau email null, ambil email publik utama via API tambahan (opsional, bisa ditambahkan di sini)
        } else {
